        ...
        <form action="{{ route('kendaraankeluar.store') }}" method="POST">
          ...
          <div class="mb-3">
            ...
            <select name="id_kendaraan_masuk" id="id_kendaraan_masuk"
                    class="form-select @error('id_kendaraan_masuk') is-invalid @enderror" required>
              <option value="">-- Pilih Kendaraan --</option>
              @foreach($parkirAktif as $item)
                <option value="{{ $item->id }}">
                  {{ $item->dataKendaraan->no_polisi }} - {{ ucfirst($item->dataKendaraan->jenis_kendaraan) }}
                </option>
              @endforeach
            </select>
...
<div class="mb-4">
  ...
  @php
    $statusOptions = ['baik', 'karcis hilang', 'kerusakan', 'kehilangan', 'merusak'];
  @endphp

  @foreach($statusOptions as $status)
    <div class="form-check">
      <input class="form-check-input" type="checkbox" name="status_kondisi[]" id="status_{{ $status }}" value="{{ $status }}">

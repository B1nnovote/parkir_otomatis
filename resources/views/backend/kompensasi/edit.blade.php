        ...
        <div class="card-body p-4">
          <form action="{{ route('kompensasi.update', $kompensasi->id) }}" method="POST">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" value="{{ $kompensasi->kendaraanMasuk->dataKendaraan->no_polisi }}" readonly>
            </div>

            <div class="mb-3">
              <label class="form-label">Jumlah Kompensasi (Rp)</label>
              <input type="number" name="jumlah" class="form-control @error('jumlah') is-invalid @enderror" value="{{ $kompensasi->jumlah }}" required>
              @error('jumlah') <div class="text-danger mt-1">{{ $message }}</div> @enderror
            </div>

            <div class="mb-3">
              <label class="form-label">Keterangan</label>
              <textarea name="keterangan" rows="3" class="form-control">{{ $kompensasi->keterangan }}</textarea>
            </div>
